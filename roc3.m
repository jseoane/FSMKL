...
%        default);
%        nonSignificantOut - Show ROC values if is not significant
%
% Output: if verbose = 1
...
fauroc = @(x,xroc) 1-1./(power(1+power(xroc/x(2),x(1)),x(3)));
...
%[cfit,resnorm] = lsqcurvefit(fauroc,st,xroc,yroc);
%xfit=linspace(0,1,500);
%yfit = fauroc(cfit,xfit);
xfit = xroc;
yfit = yroc;
...
if 1 %p<=alpha
    ...
    fitSe = lsqcurvefit(fauroc,st,table(:,1),table(:,2));
    ySe = fauroc(fitSe,xg);
    ...
    fitSp = lsqcurvefit(fauroc,st,table(:,1),table(:,3));
    ySp = fauroc(fitSe,xg);
    ...
    fauroc2 = @(x,xroc) x(4)-(x(1)-x(4))./(power(1+power(xroc/x(3),x(2)),x(5)));
    fitEff = lsqcurvefit(fauroc2,st,table(:,1),table(:,4));
    yEff = fauroc2(fitEff,xg);
    
    cop = 3;
    m=[table(CSe,1) table(CSp,1) table(CE,1) table(CEff,1)];
    if verbose
        H2=figure;
        set(H2,'Position',[570 402 868 420])
        hold on
        HSE = plot(xg,ySe,'marker','none','linestyle','-','color','r','linewidth',2);
        HCSe=plot([table(CSe,1) table(CSe,1)],[0 1],'marker','none','linestyle','--','color','r','linewidth',2);
        HSP = plot(xg,ySp,'marker','none','linestyle','-','color','g','linewidth',2);
        HCSp=plot([table(CSp,1) table(CSp,1)],[0 1],'marker','none','linestyle','--','color','g','linewidth',2);
        HEFF = plot(xg,yEff,'marker','none','linestyle','-','color','b','linewidth',2);
...
if nargout
    ...
    ROCout.table=table;
    ROCout.cotable=cotable; %confussion matrix at cost effective cut-off
    ROCout.pps=TP+TN;
    ROCout.ppn=FN+FP;
    ROCout.perf = (TP+TN)/(TP + FP + FN + TN);
